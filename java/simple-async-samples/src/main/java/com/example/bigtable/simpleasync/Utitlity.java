...
final class Utitlity {
  ...
  /**
   * Create asynchronously a table in Bigtable instance. First checks if a table with the same
   * name already exist. If yes then uniquely generates another tableName with UUID.
   */
  @VisibleForTesting
  static CompletableFuture<TableName> createTable(AsyncAdmin admin) {
    ...
    ColumnFamilyDescriptor cf_1 =
        ColumnFamilyDescriptorBuilder.newBuilder(FAMILY_1).setValue(QUALIFIER, "first-CF-value")
            .build();
    ColumnFamilyDescriptor cf_2 =
        ColumnFamilyDescriptorBuilder.newBuilder(FAMILY_2).setValue(QUALIFIER, "second-CF-value")
            .build();

    // Here AsyncAdmin#tableExist returns a future.
    System.out.printf("Checking if table with \"%s\" name already exist\n", table.getNameAsString());
    return admin.tableExists(table).thenApply(isExist -> {
      if (isExist) {
        System.out.println("Table already existed, So creating with a new name");
        return TableName.valueOf(getTableName());
      }
      System.out.printf("No table existed with \"%s\" name, now creating one\n", table.getNameAsString());
      return table;
  ...
  /**
   * This operation creates a single row and populate qualifier & values against columnFamily
   * with some values using {@link AsyncTable#put(Put)}.
   */
  @VisibleForTesting
  static CompletableFuture<Void> createSingleRowData(
    ...
    Put firstRow = new Put("first-row".getBytes())
  ...
  /**
   * This operation puts multiple rows with different qualifier & values using
   * {@link AsyncTable#putAll(List)}.
   */
  @VisibleForTesting
  static CompletableFuture<Void> createMultiRowData(
    ...
    // Creates a row with "first-scan-row" as row-key with different column, qualifier & value.
    Put firstRow = new Put("first-scan-row".getBytes())
    ...
    System.out.printf("Creating row with row-id: \"%s\" \n\n", "second-scan-now");
    // Creates a row with "second-scan-row" as row-key with different column, qualifier & value.
    Put secondRow = new Put("second-scan-row".getBytes())
