  ...
  public MetricsImpl(Credentials credentials, String projectId) throws IOException {
    ...
    requestSizes =
        meter
            ...
            .setDescription(
                "Total bytes sent per call to Bigtable service (excluding metadata, grpc and  transport framing bytes)")
            .setUnit("by")
            .ofLongs()
            .build();
    ...
    responseSizes =
        meter
            ...
            .setDescription(
                "Total bytes received per call from Bigtable service (excluding metadata, grpc and  transport framing bytes)")
            .setUnit("by")
            .ofLongs()
            .build();
        ...
        meter
            ...
            .setDescription(
                "Latency as measured by Google load balancer from the time it "
                    + "received the first byte of the request until it received the first byte of the "
                    + "response from the Cloud Bigtable service.")
            .setUnit("ms")
            .build();
